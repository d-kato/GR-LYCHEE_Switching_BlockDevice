...
FlashIAPBlockDevice_2::FlashIAPBlockDevice_2()
    : _flash(), _base(0), _size(0), _is_initialized(false), _init_ref_count(0)
{
    DEBUG_PRINTF("FlashIAPBlockDevice_2: %" PRIX32 " %" PRIX32 "\r\n", address, size);
}

FlashIAPBlockDevice_2::FlashIAPBlockDevice_2(uint32_t address, uint32_t size)
...
int FlashIAPBlockDevice_2::init()
{
    DEBUG_PRINTF("init\r\n");
...
int FlashIAPBlockDevice_2::deinit()
{
    DEBUG_PRINTF("deinit\r\n");
...
int FlashIAPBlockDevice_2::read(void *buffer,
...
int FlashIAPBlockDevice_2::program(const void *buffer,
                                 bd_addr_t virtual_address,
                                 bd_size_t size)
{
...
int FlashIAPBlockDevice_2::erase(bd_addr_t virtual_address,
                               bd_size_t size)
{
...
bd_size_t FlashIAPBlockDevice_2::get_read_size() const
{
...
bd_size_t FlashIAPBlockDevice_2::get_program_size() const
{
    if (!_is_initialized) {
        return 0;
    }
...
bd_size_t FlashIAPBlockDevice_2::get_erase_size() const
{
    if (!_is_initialized) {
        return 0;
    }
...
bd_size_t FlashIAPBlockDevice_2::get_erase_size(bd_addr_t addr) const
{
    if (!_is_initialized) {
        return 0;
    }
...
bd_size_t FlashIAPBlockDevice_2::size() const
{
